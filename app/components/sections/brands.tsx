...
const BrandCard = styled.a<{
  ...
  background-color: ${props => props.$data.blocks?.setting?.cardBackground || "#FFFFFF"};
...
const BrandName = styled.span<{
  ...
  font-size: ${props => props.$data.blocks?.setting?.brandNameFontSize || "16"}px;
  font-weight: ${props => props.$data.blocks?.setting?.brandNameFontWeight || "normal"};
  margin-top: 0.5rem;
`;
...
export const Brands: React.FC<BrandsProps> = ({
  ...
  return (
    ...
    >
      ...
      <BrandsGrid ref={containerRef} $data={sectionData} $isMobile={preview === "sm"} dir="rtl">
      {sectionData.blocks?.brands.map((brand) => (
          <BrandCard className="border-l p-3" key={brand.id} $data={sectionData}>
