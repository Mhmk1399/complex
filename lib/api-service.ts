...
class ApiService {
  ...
  useGet(endpoint: string, options: ApiConfig & { revalidateOnFocus?: boolean } = {}) {
    const { cache = 60000, revalidateOnFocus = false, ...fetchOptions } = options
    
    return useSWR(
      endpoint,
      () => this.get(endpoint, fetchOptions),
      {
