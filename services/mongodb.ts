...
export async function fetchFromMongoDB(route: string, mode: string, storeId: string): Promise<string> {
...
export async function saveToMongoDB(route: string, mode: string, storeId: string, newLayout: any): Promise<void> {
  
  const existing = await Jsons.findOne({ storeId, route });
  
  if (existing) {
    const updateField = mode === 'lg' ? { lgContent: newLayout } : { smContent: newLayout };
    await Jsons.findOneAndUpdate(
      { storeId, route },
      updateField,
      { new: true }
    );
...
export async function listMongoDBTemplates(storeId: string) {
  ...
  const files = docs.flatMap(doc => [`${doc.route}lg.json`, `${doc.route}sm.json`]);
  return { files };
}

export async function createNewMongoJSON(routeName: string, storeId: string): Promise<void> {
  const defaultData = {
    children: {
      type: "about",
      metaData: {
...
export async function deleteMongoDBFile(routeName: string, storeId: string): Promise<void> {
